...
class GeneralController extends WebController
{
    ...
    public function login(Request $request)
    {
        ...
        $creds = [$find_field => $request->username, 'password' => $request->password, 'status' => 'active', 'type' => 'admin'];
        if (Auth::attempt($creds, $remember)) {
            // if (Auth::user()->type == "user") {
            //     Auth::logout();
            //     return back()->with('error', 'You dont have permission to access Admin Panel');
            // }
    ...
    public function Admin_dashboard(Request $request)
    {
        ...
        $senderUser = User::where('type','user')->where('is_sender',1)->count();
        $receiverUser = User::where('type','user')->where('is_sender',0)->count();

        $total = Transaction::where('transaction_type','admin')->where('is_withdraw',0)->sum('amount');

        $pendingWithdra = Transaction::where('transaction_type','admin')->where('is_withdraw',1)->where('payment_status','pendding')->sum('amount');
        $approveWithdra = Transaction::where('transaction_type','admin')->where('is_withdraw',1)->where('payment_status','success')->sum('amount');
        ...
        return view('admin.general.dashboard', [
            ...
            'total' => $total,
            'pendingWithdra' => $pendingWithdra,
            'approveWithdra' => $approveWithdra,
...
//        return redirect()->route('admin.login');
    ...
    public function content($slug = 'about_us',$lang= 'en'){


        $content = Content::where('slug',$slug)->first();
        if($content){
            return view('general.contentpage', [
                'content' => $content,
                'title' => 'Content',
                'header_panel' => false,
            ]);
